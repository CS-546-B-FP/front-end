  ...
  try {
    ...
    if (!result.ok) {
      return res.render('auth/login', {
        pageTitle: 'Login — LeaseWise NYC',
        formData: { username },
        errors: [{ message: result.error || 'Invalid username or password.' }]
      });
    }
...
router.post('/register', async (req, res) => {
  ...
  try {
    const result = await api.auth.register({ firstName, lastName, email, username, password });

    if (!result.ok) {
      return res.render('auth/register', {
        pageTitle: 'Register — LeaseWise NYC',
        formData: { firstName, lastName, email, username },
        errors: [{ message: result.error || 'Registration failed.' }]
      });
    }
