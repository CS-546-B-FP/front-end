...
router.get("/watchlist", async (req, res) => {
  try {
    ...
    const buildingPromises = watchlistIds.map((id) =>
      api.buildings.getById(id),
    );
    ...
    const buildings = results
      ...
      .map((r) => ({
        ...r.data,
        address: r.data.streetAddress || r.data.address || "",
      }));
...
router.get("/shortlists/:id", async (req, res) => {
  try {
    ...
    if (!result.ok) {
      return res
        .status(404)
        .render("errors/404", { pageTitle: "Not Found — LeaseWise NYC" });
    }

    const shortlist = (result.data || []).find(
      (sl) => String(sl._id) === req.params.id,
    );
    if (!shortlist) {
      return res
        .status(404)
        ...
        return {
          ...
          building: bResult.ok
            ? {
                ...bResult.data,
                address:
                  bResult.data.streetAddress || bResult.data.address || "",
              }
    ...
    return res
      .status(500)
      .render("errors/500", { pageTitle: "Error — LeaseWise NYC" });
  }
});
  ...
  try {
    ...
    const buildingResults = await Promise.all(
      (shortlist.items || []).map((item) =>
        api.buildings.getById(item.buildingId || item._id),
      ),
    );
...
router.delete("/shortlists/:id/items/:buildingId", async (req, res) => {
  try {
    const cookie = req.session.backendCookie;
    await api.shortlists.removeItem(
      req.params.id,
  ...
  try {
    ...
    if (result.ok) {
      ...
      if (result.data.watching) {
        if (!ids.includes(req.params.buildingId))
          ids.push(req.params.buildingId);
      } else {
        req.session.watchlistIds = ids.filter(
          (id) => String(id) !== String(req.params.buildingId),
        );
      }
